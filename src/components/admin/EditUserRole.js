...
function EditUserRole() {
  ...
  //     var axios = require("axios");
  //     axios
  //       .get(`${window.backendURL}/admin/get-division-management-permissions`) //get the id and name of all the division managment permissions
  //       .then((res) => {
  ...
  //     var axios = require("axios");
  //     axios
  //       .get(`${window.backendURL}/admin/get-employee-management-permissions`) //get the id and name of all the employee managment permissions
  //       .then((res) => {
  ...
  //     var axios = require("axios");
  //     axios
  //       .get(`${window.backendURL}/admin/get-task-management-permissions`) //get the id and name of all the task managment permissions
  //       .then((res) => {
  ...
  const saveChanges = () => {
    var axios = require("axios");
    axios
      .post(`${window.backendURL}/admin/update-user-role`, {
        //update user role and permissions (Division, Employee and Task)
        userRoleId: editRole,
  ...
  const deleteUserRole = () => {
    var axios = require("axios");
    axios
      .delete(`${window.backendURL}/admin/delete-user-role/${editRole}`) //delete the user role record in the DB under the given user role Id
      .then((res) => {
        console.log(res);
  ...
  const handleDivPermissionChange = (event) => {
    ...
    if (event.target.checked) {
      if (!roleDivPermissions.includes(value))
        setRoleDivPermissions([...roleDivPermissions, value]);
    } else {
      setRoleDivPermissions(
        roleDivPermissions.filter((item) => item !== value)
      );
    }
  };
  ...
  const handleEmpPermissionChange = (event) => {
    let value = event.target.value;
    if (event.target.checked) {
      if (!roleEmpPermissions.includes(value))
        setRoleEmpPermissions([...roleEmpPermissions, value]);
    } else {
      setRoleEmpPermissions(
        roleEmpPermissions.filter((item) => item !== value)
      );
    }
  };
  ...
  const handleTaskPermissionChange = (event) => {
    let value = event.target.value;
    if (event.target.checked) {
      if (!roleTaskPermissions.includes(value))
        setRoleTaskPermissions([...roleTaskPermissions, value]);
    } else {
      setRoleTaskPermissions(
        roleTaskPermissions.filter((item) => item !== value)
      );
    }
  };
  ...
  let userRoleList = userRoles.map((userRole, index) => {
    return (
      <MenuItem key={"role" + index} value={userRole.id}>
        {userRole.name}
      </MenuItem>
    );
  });
